...
Like many long-running open source sites, it’s grown organically over time. With billions of downloads across all its libraries, the site had accumulated the kind of technical debt that’s easy to miss in incremental changes but obvious in aggregate: unused JavaScript lingering in the bundle, CSS that no longer mapped to any UI, outdated linting rules (and packages), and performance regressions that can’t be attributed to a single commit or feature.

Performance wasn’t “broken” in an obvious way. It had degraded slowly – through unused code paths, layered abstractions, and tooling that hadn’t been kept to pace with how the site actually behaved in the browser.

At the same time, Tanstack, like many OSS projects, [relies on ads as a form of revenue to keep the lights on](https://tanstack.com/ads). There isn’t a consistent, guaranteed income stream backing the work. That means that ads aren’t an optional enhancement or a late-stage add-on, they’re a non-negotiable constraint that the front end has to be designed around.
...
Ads execute in the same JavaScript environment. They compete for the same main thread. They participate in the same layout and style calculations. They contend for the same network bandwidth and memory budget as your UI.

Treating ads as external — something you “integrate” and then mentally exclude from the system — is how performance problems slip through code review unnoticed.

When regressions show up, they’re often attributed to vague causes (“the site just feels heavier now”) instead of being traced back to **resource contention**. But browsers are schedulers. They don’t care _why_ work is happening — only when it happens and what it blocks.
...
Performance budgets are often reduced to numeric targets — a Lighthouse score to hit, a bundle size to stay under, a single metric to optimize. I’ve been guilty of this myself.

In practice, budgets are more useful as **decision-making constraints**.

Every ad slot introduces tradeoffs that force real questions:

- What work is allowed on the main thread during initial render?
- Which scripts are permitted before content is stable?
- How much layout instability is acceptable for a reading experience?
- What _must_ be guaranteed before monetization loads?

On documentation and content-heavy sites, these tradeoffs matter more than raw throughput. Reader intent is focused and task-driven. Small delays are more noticeable. Layout shifts break trust quickly.
...
Once ads are acknowledge as part of the performance budget, the conversation changes.
...
Readers are scanning, reading, and comparing information. They notice when text jumps. They feel delays more acutely because the interaction model is simple. There’s less visual noise to hide instability.
...
Containment means ensuring that the system’s needs don’t silently degrade the rest of the experience. It means accepting constraints upfront instead of rediscovering them through regressions. It means designing for longevity, not just launch-day success.
...
Performance isn’t just a technical concern. It’s a product promise — one that reflects how much a project values users’ time and attention.
