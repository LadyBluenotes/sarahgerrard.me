...
Next is to define the `LinkedList` that manages the nodes and their connections. The `LinkedList` class has three properties:

- `head`: represents the first node
- `tail`: represents the last node
- `size`: represents the number of nodes present

By default, all three properties are set to `0` or `null`.
